...
//      Cor := clGrayText;
    ...
    begin
      ...
      Relacao.Nome := GeraBaseNome('Relação');
      SetBounds(Clicado.X, Clicado.Y, Width, Height);
      Result := Me;
      if UsrSelA is TBaseEntidade then
...
//        TLigacao(LigaTab.FLigacoes[0]).FCard.Cardinalidade := 1;
    ...
    begin
      ...
      begin
        ...
        TabelaP.Nome := TabelaP.Nome + '+' + T.Nome;
        T.associacao.associacao := TabelaP;
        FreeAndNil(T);
      end;
    end else
...
begin
  Result := true;
  i := 0;
  while (i < oModelo.ComponentCount -1) do
  begin
...
begin
  ...
  begin
    ...
    FRelacao.SendToBack;
    FRelacao.Nome := Modelo.GeraBaseNome('Relação');
    FRelacao.Pai := self;
  end;
...
begin
  try
    ...
    Writer.write('Criação de novo esquema', false, True);
    FModelos.Add(Result);
    if novo or (Arq = '') then
    begin
      novo := true;
...
function TVisual.LoadModeloByBin(arq, onErroNome: string;
  ...
  var Mem: TMemoryStream;
      myVersao: string[5];
begin
  Writer.write('Abrindo arquivo "' + arq + '"...', false, true);
  ...
  Writer.write('Fim do processo de abertura do arquivo "' + arq + '"', false, true);
end;
...
begin
  ...
  begin
    ...
    begin
      Brush.Color := clWhite;
      lNome := ControlaCaption(Canvas, Self.Width -2 -TextWidth('***'), Nome + ': ' + tipo);
      Pen.Style:=psSolid;
      Pen.Color := TColor(-5);
      Rectangle(Rect);
      with Rect do
